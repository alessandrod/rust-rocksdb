...
impl DBPinnableSlice<'_> {
    /// Releases pinned RocksDB resources and clears the current value.
    ///
    /// Any capacity allocated for a copied value is retained for reuse by a
    /// subsequent `get_pinned_into` operation.
    ...
    ///
    /// # Unsafe
    /// Requires an owned pointer returned by `rocksdb_get_pinned` or
    /// `rocksdb_pinnableslice_create`.
